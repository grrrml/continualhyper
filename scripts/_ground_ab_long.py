"""Test A/B groundingu: ta sama generacja z ramka lewo/prawo/bez.
Piksele identyczne => grounding nie strzela przy inferencji (bug okablowania).
Generacja skopiowana 1:1 z scripts/_box_probe.py."""
import sys, torch
sys.path.insert(0, ".")
...
cfg = load_config("configs/phaseP/P_ground_long.yaml")
...
manager = build_hyper(bundle, target_modules=tuple(cfg.get("target_modules", DEFAULT_TARGETS)),
                      ...
                      **cfg.get("hyper", {}))
load_hyper(manager, "outputs/phaseP/P_ground_long/hyper.pt", map_location="cuda")
...
tm = token_span_mask(bundle.tokenizer, [prompt], "dog").cuda() if cfg.get("token_mask_lora") else None

imgs = {}
for tag, box in [("L", (0.25, 0.5, 0.5, 1.0)), ("R", (0.75, 0.5, 0.5, 1.0)), ("OFF", None)]:
    manager.cond_box = box
    manager.set_ground(0, box)
    ...
    Image.fromarray((img.permute(1, 2, 0).clamp(0, 1) * 255).byte().cpu().numpy()
                    ).save(f"outputs/probe/ab_long_{tag}.png")

dLR = float((imgs["L"] - imgs["R"]).abs().mean())
dLO = float((imgs["L"] - imgs["OFF"]).abs().mean())
print(f"|L-R| = {dLR:.5f}  |L-OFF| = {dLO:.5f}   (0.00000 = grounding NIE dziala przy inferencji)")
