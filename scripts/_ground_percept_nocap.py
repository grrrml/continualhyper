...
cfg = load_config("configs/phaseP/P_ground_gsa_nocap.yaml")
...
manager = build_hyper(bundle, target_modules=tuple(cfg.get("target_modules", DEFAULT_TARGETS)),
                      ...
                      **cfg.get("hyper", {}))
load_hyper(manager, "outputs/phaseP/P_ground_gsa_nocap/hyper.pt", map_location="cuda")
manager.eval(); manager.lora_scale = 0.7
set_grounded(bundle.unet, manager)
dino = _Dino("cuda")
os.makedirs("outputs/probe/percept_nocap", exist_ok=True)
        ...
        for qname, box in Q.items():
            ...
            for i in range(N):
                ...
                if c["concept_id"] == "cifc_duck_toy" and i == 0:
                    pil.save(f"outputs/probe/percept_nocap/k{kap}_s{sched}_{qname}.png")
