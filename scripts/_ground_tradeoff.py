"""Trade-off placement vs tozsamosc po poprawkach 1+2 (cond-only + harmonogram kappa).
Dla (kappa, sched): sonda cwiartkowa (przypadek 25%) + DINO calego obrazu vs referencje.
"""
...
GRID = [(2.0, 1.0), (2.0, 0.4), (2.0, 0.3), (3.0, 0.3), (4.0, 0.3)]
N = 3
...
dino = _Dino("cuda")
os.makedirs("outputs/probe/tradeoff", exist_ok=True)
        ...
        for qname, box in Q.items():
            ...
            for i in range(N):
                ...
                if c["concept_id"] == "cifc_duck_toy" and i == 0:
                    pil.save(f"outputs/probe/tradeoff/k{kap}_s{sched}_{qname}.png")
    print(f"kappa={kap} sched={sched}: cwiartki {ok}/{tot} = {ok/tot:.1%} | DINO(caly obraz) {dsum/tot:.4f}", flush=True)
print("TRADEOFF_DONE", flush=True)
